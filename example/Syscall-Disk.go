...
func main() {
	NtAllocateVirtualMemory, err := hades.GetSysidDisk("NtAllocateVirtualMemory")
	if err != nil {
		panic(err)
	}
	...
	NtWriteVirtualMemory, err := hades.GetSysidDisk("NtWriteVirtualMemory")
	if err != nil {
		panic(err)
	}
	...
	var Thread uintptr
	NtCreateThreadEx, err := hades.GetSysidDisk("NtCreateThreadEx")
	if err != nil {
		panic(err)
	}
	...
	Time := -(INFINITE)
	NtWaitForSingleObject, err := hades.GetSysidDisk("NtWaitForSingleObject")
	if err != nil {
		panic(err)
	}
